...
LogindIntegration *LogindIntegration::self()
{
    if (!s_self)
        s_self = new LogindIntegration(QDBusConnection::systemBus());
    return s_self;
}
...
void LogindIntegration::getVirtualTerminal()
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this,
        [this](QDBusPendingCallWatcher *self) {
            ...
            self->deleteLater();
            if (!reply.isValid()) {
                qDebug() << "Failed to get VTNr Property of " << m_sessionControllerName << " session:" << reply.error().message();
                return;
            }
...
void LogindIntegration::getSeat()
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this,
        [this](QDBusPendingCallWatcher *self) {
            ...
            self->deleteLater();
            if (!reply.isValid()) {
                qDebug() << "Failed to get Seat Property of " << m_sessionControllerName << " session:" << reply.error().message();
                return;
            }
            DBusLogindSeat seat = qdbus_cast<DBusLogindSeat>(reply.value().value<QDBusArgument>());
            const QString seatPath = seat.path.path();
            qDebug() << m_sessionControllerName << " seat:" << seat.name << "/" << seatPath;
            qDebug() << m_sessionControllerName << " seat:" << seat.name << "/" << seatPath;
